...
app.use(cors({
  origin: process.env.NODE_ENV === 'production' 
    ? ['http://localhost:5173', 'https://yourdomain.com'] // Update with your domain
    : true
}));
...
app.post('/api/waitlist', async (req, res) => {
  try {
    ...
    let referredBy = null;
    if (referral_code) {
      const referrer = await database.getWaitlistEntryByReferralLink(referral_code);
      if (referrer) {
...
app.post('/api/users/:userId/achievements', async (req, res) => {
  try {
    ...
    await database.createUserAchievement(userId, achievement_type, points_earned);
